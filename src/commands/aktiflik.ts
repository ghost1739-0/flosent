...
const AKTIFLIK_CHANNEL_ID = process.env.AKTIFLIK_CHANNEL_ID ?? '1500135056637689938';
const AKTIFLIK_ROLE_ID = process.env.AKTIFLIK_ROLE_ID ?? '1500135055207567590';
const AKTIFLIK_PANEL_CHANNEL_ID = process.env.AKTIFLIK_PANEL_CHANNEL_ID ?? '1500135056440819836';
const AKTIFLIK_PANEL_PERM_ROLE_ID = process.env.AKTIFLIK_PANEL_PERM_ROLE_ID ?? '1500135055148843147';
const LOG_CHANNEL_ID = process.env.LOG_CHANNEL_ID ?? '1500135056637689938';
...
const command: BotCommand = {
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    try {
      ...
      console.log(`[Aktiflik] Süre değeri alındı: ${hours} saat (${durationSeconds} saniye)`);
      ...
      console.log(`[Aktiflik] Zamanlayici kuruldu: ${hours} saat (${durationSeconds} saniye, ${durationMs} ms)`);
      setTimeout(() => {
